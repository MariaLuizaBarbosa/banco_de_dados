...
/* - COUNT -> contar numero de linhas
  ...
  -- contar quantas pizzas
   ...
   -- QUESTÃO 3    -->  Liste os pedidos feitos após '2016-12-15' com o nome do cliente.
	...
	select nome, pedidos.id as pedidos from clientes right join pedidos on clientes.id = pedidos.cliente_id;

    -- QUESTÃO 6   -->   Qual o valor total de todos os pedidos feitos até agora?
	select sum(valor) from itens_pedido where pedido_idz;
    
    -- QUESTÃO 7  -->   Qual o total gasto por cada cliente?
     select clientes.id, clientes.nome, sum(pedidos.valor) as valor_gasto from clientes inner join pedidos on pedidos.cliente_id = clientes.id group by clientes.id, clientes.nome;
    ...
    -- QUESTÃO 8  -->  Liste todos os clientes e seus pedidos feitos no mês de dezembro de 2016
     ...
     -- QUESTÃO 10  -->  . Liste o valor total dos pedidos por cliente, incluindo pedidos feitos por clientes que foram excluídos 
    ...
    -- QUESTÃO 13  -->  Quantos pedidos cada cliente fez (mesmo que não tenham feito nenhum)?
        select nome, count(pedidos.id) as quantidade_pedido from clientes left join pedidos on pedidos.cliente_id = clientes.id group by nome;
    ...
    select nome, max(valor), min(valor) from clientes left join pedidos on pedidos.cliente_id = clientes.id where clientes.nome = 'Laura Madureira' group by nome;
    ...
    inner join clientes on pedidos.cliente_id = clientes.id group by clientes.nome;
    select * from pedidos;
    ...
    -- QUESTÃO 20  --> Liste todos os clientes, mesmo que não tenham feito pedidos, com seus respectivos pedidos
    select clientes.nome, count(pedidos.id) as total_pedidos from pedidos right join clientes on clientes.id = pedidos.cliente_id group by clientes.id, clientes.nome;
    
    select clientes.nome, count(pedidos.id) as total_pedidos 
    from clientes 
    left join pedidos on clientes.id = pedidos.cliente_id group by clientes.id, clientes.nome;
    ...
    select nome, sum(valor) as total from pedidos inner join clientes on clientes.id = pedidos.cliente_id group by nome order by total desc limit 3;
