...
/* - COUNT -> contar numero de linhas
  ...
  -- contar quantas pizzas
   ...
   -- inner join = retorn registros quando há pelo menos uma correspondência em ambas tabelas
    select * from clientes as c inner join pedidos as pedidos on pedidos.cliente_id = clientes.id;
    
    
    -- left join --> retorna os registros da tabela da esquerda (1° tabela mencionada) e os registros correspondem a segunda tabela da direita (2° tabela mencionada)
    ...
    -- right join --> retorna odos os registros da tablea da direita(2° tabela)
